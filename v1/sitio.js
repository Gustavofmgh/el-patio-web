// Cabecera, pie y bloques que salen de datos.js (versión Cuaderno).
(() => {
  ...
  document.body.insertAdjacentHTML('afterbegin', `<header class="cabecera"><div class="env">
    <a class="marca" href="index.html"><img src="../img/logo.png" alt="">El Patio</a>
  ...
  document.body.insertAdjacentHTML('beforeend', `<div class="comunidad"><div class="env">
      <h2>Pasa, siéntate.<br>Estás en El Patio.</h2>
  ...
  P.pinta({
    caras: () => P.ponentes.map(p => `<li><a href="ponentes.html#${p.id}">
      <img src="${foto(p)}" alt="${e(p.nombre)}" loading="lazy"><h3>${e(p.nombre)}</h3><p class="gancho">«${e(p.gancho)}»</p></a></li>`).join(''),
    ...
    proximamente: () => `<div class="n">${P.pendientes()}</div><div><h2>conversaciones grabadas que todavía no han salido</h2>
    ...
    historial: () => P.ponentes.filter(p => p.asistentes).slice().reverse().map(p => `<li><a href="ponentes.html#${p.id}">
      <span class="n">Patio ${p.patio}</span><div><strong>${e(p.nombre)}</strong><em>${e(p.charla.titulo)}</em></div><span>${p.asistentes} personas</span></a></li>`).join(''),
